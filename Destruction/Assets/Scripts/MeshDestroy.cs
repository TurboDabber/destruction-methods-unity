...
public class MeshDestroy : MonoBehaviour
{
    ...
    public void DestroyMesh()
    {
        ...
        for (var i = 0; i < parts.Count; i++)
        {
            ...
            parts[i].GameObject.transform.SetParent(parent.transform);

            //parts[i].GameObject.GetComponent<Rigidbody>().AddForceAtPosition(parts[i].Bounds.center, transform.position);
        }

        Destroy(gameObject);
        UnityEngine.Debug.Log("Dividing set up: " + stopwatch.ElapsedMilliseconds);
    }
